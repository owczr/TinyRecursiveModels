...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings, base, device=None):
        ...
        # RoPE
        inv_freq = 1.0 / (
            base ** (torch.arange(0, dim, 2, dtype=FLOAT_DTYPE, device=device) / dim)
        )
        t = torch.arange(max_position_embeddings, dtype=FLOAT_DTYPE, device=device)
...
def rms_norm(hidden_states: torch.Tensor, variance_epsilon: float) -> torch.Tensor:
    ...
    hidden_states = hidden_states.to(FLOAT_DTYPE)
